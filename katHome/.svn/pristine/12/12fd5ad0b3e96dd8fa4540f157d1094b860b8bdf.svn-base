...
<script type="text/javascript" src="${jsUrl}admin/statistice_county.js"></script>
...
<section class="container">
	<div class="topic_form">
		...
		<ul class="location">
			...
			<li><a href="/admin/statisticsCountry" title="국가별 접속통계" class="present">국가별 접속통계</a></li>
		</ul>
	</div>
	<div class="content">
		...
		<table class="tb1" summary="국가별 접속통계">
			<caption>국가별 접속통계 테이블</caption>
			<colgroup>
				<col />
				<col />
			</colgroup>
			<thead>
				<tr style="cursor:pointer">
					<th scope="col" onclick="tableSort(county_contents,0);" title="Country">Country<a href="javascript:;" id="sort_0" class="sort"></a></th>
					<th scope="col" onclick="tableSort(county_contents,1);" title="Users">Users<a href="javascript:;" id="sort_1" class="sort"></a></th>
				</tr>
			</thead>
			<tbody id="county_contents">
			</tbody>
		</table>
	</div>
</section>
...
<script>
	...
	function queryTotalReporting() {
		...
		gapi.client.analytics.data.ga.get({
	    	...
	    	$(".sort").removeClass("btn_down");
			$(".sort").removeClass("btn_up");
	    	$("#county_contents").html("");
	    	$("#county_contents").html("<tr><td colspan='2'>No Data</td></tr>");
	    	countryCreateTable(json_parse);
	  	}).then(null, function(err) {
	      // Log any errors.
	      console.log(err);
	  	});
